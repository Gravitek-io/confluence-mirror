...
function extractHeadings(
  ...
  addItem: (item: { id: string; title: string; level: number }) => void,
  ...
  if (node.content) {
    node.content.forEach((child) =>
      extractHeadings(child, addItem, existingSlugs)
    );
  }
}
...
export default function ADFRendererWithToc({
  document,
  pageId,
}: ADFRendererWithTocProps) {
  ...
  return <div>{renderADF(document, undefined, { pageId })}</div>;
}
