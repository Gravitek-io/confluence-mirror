...
export default async function ShowroomContent({ document }: ShowroomContentProps) {
  // Process the document with media (for showroom special handling)
  const mediaProcessedDocument = await processADFWithMedia(document, '', 'showroom');
  
  // Pre-process the document for TOC extraction
  const { enrichedDocument, tableOfContents } = processADFWithTOC(mediaProcessedDocument);
