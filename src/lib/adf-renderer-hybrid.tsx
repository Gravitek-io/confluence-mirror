...
import ConfluenceImageHybrid from '@/components/confluence-image-hybrid';
...
export function renderADFHybrid(node: ADFNode | ADFDocument, key?: string | number, options?: RenderOptions): React.ReactNode {
  if (!node) return null;
    ...
    case 'doc':
      return (
        <div key={key} className="adf-document">
          {node.content?.map((child, index) => renderADFHybrid(child, index, options))}
        </div>
      );
        ...
        <li key={key} className="mb-2">
          {node.content?.map((child, index) => {
            ...
            if (child.type === 'paragraph') {
              return (
                <span key={index}>
                  {child.content?.map((textChild, textIndex) => renderADFHybrid(textChild, textIndex, options))}
                </span>
              );
            }
            return renderADFHybrid(child, index, options);
          })}
        </li>
      );
