...
import { renderADFHybrid, ADFNode, ADFDocument } from '@/lib/adf-renderer-hybrid';
...
interface ADFRendererWithTocHybridProps {
  document: ADFDocument;
  pageId: string;
}

export default function ADFRendererWithTocHybrid({ document, pageId }: ADFRendererWithTocHybridProps) {
  ...
  useEffect(() => {
    ...
    if (!hasExtracted.current || documentRef.current !== document) {
      console.log('Extracting headings from document (hybrid):', document);
      clear();
      const existingSlugs = new Set<string>();
      extractHeadings(document, addItem, existingSlugs);
      documentRef.current = document;
      hasExtracted.current = true;
      console.log('Headings extraction completed (hybrid)');
    }
  });

  return (
    <div>
      {renderADFHybrid(document, undefined, { pageId })}
    </div>
  );
}